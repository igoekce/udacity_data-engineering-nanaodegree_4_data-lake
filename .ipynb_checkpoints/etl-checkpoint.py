...
def process_song_data(spark, input_data, output_data):
    """
    Songfiles: Load data from S3 source, create tables and write back the data as parquet files to s3.
    
    Parameters
    ----------
    spark: session
          variable set for spark session
    input_data: path
           path to song_data s3 bucket
    output_data: path
            path to the parquet files to write
    """
    # get filepath to song data file
    song_data = input_data + 'song_data/A/A/A/*.json' # the project instruction are not clear as it is recommended to use a sample dataset
...
def process_log_data(spark, input_data, output_data):
    """
    Logfiles: Load data from S3 source, create tables and write back the data as parquet files to s3.
...
def main():
    """
    S3 Bucket to be existent, otherwise script will fail
    """

    spark = create_spark_session()
    input_data = "s3a://udacity-dend/"
    output_data = "s3a://output-data-igo/"
